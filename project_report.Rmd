...
ggplot(df, aes(x=Agegroup, y=Percentage)) + 
  ...
  geom_text(aes(label = round(Percentage,1)), vjust = -0.2, size = 4,
            position = position_dodge(0.9),color ="rosybrown3") + ggtitle("Fig4. Percentage of Diabetes in Different Age Groups")+
...
ggplot(df, aes(x=Pregnancies, y=Percentage)) + 
  ...
  geom_text(aes(label = round(Percentage,1)), vjust = -0.2, size = 3,
            position = position_dodge(0.9),colour = "rosybrown4") + ggtitle("Fig3. Percentage of Diabetes According to Different Pregnancies")+
  theme_minimal()+
  theme(plot.title = element_text(color = "darkgrey",size = 12))
```
...
Full model test stat

The full model result shows `Pregnancies`, `Glucose`,  `BloodPressure`, `BMI`, and `DiabetesPredigreeFunction` are significantly related  to diabetes odds, while `SkinThickness`, `Insulin` and `Age` are not significantly related to diabetes odds. From the odds of each variable, we expect that the diabetes associated with pregnancies, glucose, blood pressure, BMI, Diabetes Pedigree Function and age, since the full model gives the estimates is rather larger for these five variables.  The full model gives an AIC around 783. Due to the nonsignificant variables included in the model and high AIC, we need to adjust our model.
and the obtained test statistics gives R square equals to 0.265, and Alc is around 740 which indicates a lower model fitting rate, but the test accuracy is around 0.79 suggests a good prediction rate. Due to the contradiction of lower R square value and higher accuracy, other model fitting is needed. Thus, the AIc selection model was used to achieve a higher R square value. 
...
The second model was fitted using best subset regression that compares all possible models and returns the two best models for one predictor, two predictors, three predictors, and so on for the number of possible predictors that were entered into the best subsets regression. The best model it generated includes seven predictors: ‘`Pregnancies`, `Glucose`,  `BloodPressure`, `BMI`, and `DiabetesPredigreeFunction`, and `Age`, and `Insulin`. 
...
RMSE is the square root of the averaged squared difference between the actual value and the predicted value of the target variable. It gives the average prediction error made by the model, thus decreasing the RMSE value increases the accuracy of the mode. 
...
### Model Comparison:
The stepwise model observed six variables which helps predict diabetes, where the best subset model gives seven variables. The statistical measurement results were shown through the table of model comparison. We split the data set so that 80% is used for training a regression model and 20% is used to evaluate the model performance. The obtained AIc result , AUC result and accuracy were rather same, and all other criteria are similar, thus, the simplest  model–stepwise regression– was chosen as the final prediction model.

### Was the finding as expected?
